...
bool loadObjectsFromFile(string filename, Cloth *cloth, ClothParameters *cp, vector<CollisionObject *>* objects, int sphere_num_lat, int sphere_num_lon) {
  ...
  for (json::iterator it = j.begin(); it != j.end(); ++it) {
    ...
    if (key == CLOTH) {
      ...
      cloth->sim_frequency = 300;
      cloth->sim_amplitude = 0.15;
      cloth->sim_damper = 0;

      cloth->sim_p.resize(num_width_points, vector<double>(num_height_points, 0.0));
      cloth->sim_p_prev.resize(num_width_points, vector<double>(num_height_points, 0.0));
      cloth->sim_p_next.resize(num_width_points, vector<double>(num_height_points, 0.0));
      ...
      // Create a list to hold the spheres
      size_t num_particles = 1000;
      for(int i=0; i<num_particles; i++){
        // Add the sphere to the list
        // Randomly set the position for each point
          double random_x = ((double)rand() / (RAND_MAX));
          double random_y = ((double)rand() / (RAND_MAX));
        Vector3D *modified_origin = new Vector3D(cloth->width * random_x, origin.y, cloth->height * random_y);
        Sphere *s = new Sphere(*modified_origin, radius, friction, sphere_num_lat, sphere_num_lon);
        objects->push_back(s);
      }
    } else { // PLANE
