  ...
  if (!is_paused) {
    ...
    for (int i = 0; i < simulation_steps; i++) {
      ...
      for (auto& collision_object : *collision_objects) {
        // std::string parent_type = typeid(*collision_object).name();
        // if (parent_type == "struct Sphere") {
          Sphere* sphere = dynamic_cast<Sphere*>(collision_object);
          sphere->simulate(frames_per_sec, simulation_steps, external_accelerations, cloth);
        // }
      }
    }
  }
...
void ClothSimulator::drawWireframe(GLShader &shader) {
  ...
  for (int i = 0; i < cloth->springs.size(); i++) {
    ...
    Vector3D na = { 0.0, 1.0, 0.0 };
    Vector3D nb = { 0.0, 1.0, 0.0 };
...
void ClothSimulator::initGUI(Screen *screen) {
...

  {
    ...
    fb->setFontSize(14);
    fb->setValue(cloth->sim_amplitude);
    fb->setSpinnable(true);
    fb->setCallback([this](float value) {
        cloth->sim_amplitude = value;

        for (auto& subVec : cloth->sim_p) {
            std::fill(subVec.begin(), subVec.end(), 0.0);
        }

        for (auto& subVec : cloth->sim_p_prev) {
            std::fill(subVec.begin(), subVec.end(), 0.0);
        }

        for (auto& subVec : cloth->sim_p_next) {
            std::fill(subVec.begin(), subVec.end(), 0.0);
        }
    });

    new Label(panel, "Frequency (Hz) :", "sans-bold");

    fb = new FloatBox<double>(panel);
    fb->setEditable(true);
    fb->setFixedSize(Vector2i(100, 20));
    fb->setFontSize(14);
    fb->setValue(cloth->sim_frequency);
    fb->setSpinnable(true);
    fb->setCallback([this](float value) { 
      cloth->sim_frequency = value; 
